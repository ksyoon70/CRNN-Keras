...
def get_model_path(model_type, backbone="crnn"):
...
# Get the model
model = get_Model(training=True,categories_len=len(char_to_num.get_vocabulary()),img_shape=[img_width,img_height,1])
...
model_sub_path_str = get_model_path('LSTM')
...
model_save_filename = "LSTM_epoch_{}_val_loss_{:.4f}.h5".format(datetime.now().strftime("%Y%m%d-%H%M%S"),val_loss[-1])
